...
export function TourItinerarySection({ form }: TourItinerarySectionProps) {
        ...
        {itinerary?.map((item, index) => (
          <div key={index} className="flex items-start justify-between rounded border p-2">
            ...
            <div className="flex gap-1">
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleEdit(index)} type="button">
                <Edit className="h-3 w-3" />
              </Button>
              <Button variant="ghost" size="icon" className="h-6 w-6 text-red-500" onClick={() => handleDelete(index)} type="button">
