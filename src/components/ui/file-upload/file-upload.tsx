...
import { ROOT_NAME, StoreContext, FileUploadContext } from './contexts';
import { DirectionContext, createStore, useAsRef, useDirection, useLazyRef } from './store';
import type { Direction, FileUploadRootProps } from './types';

export const FileUploadRoot = React.forwardRef<HTMLDivElement, FileUploadRootProps>((props, forwardedRef) => {
  const {
    value,
    defaultValue,
    onValueChange,
  ...
  const files = useLazyRef<Map<File, import('./types').FileState>>(() => new Map()).current;
  ...
  const contextValue = React.useMemo<import('./types').FileUploadContextValue>(
    () => ({
      dropzoneId,
      inputId,
      listId,
